...
export default function LearningEngine({ predictionContext, language }) {
          ...
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
              <h2 style={{ margin:0, fontSize:'1.2rem', fontWeight:900, color:'#1d1d1f' }}>
                {t.le_auto_calibrating}
              </h2>
              <span className="le-tag pulse-tag">{t.le_live_learning}</span>
              {isPrePopulated && (
            ...
            <p style={{ margin:'4px 0 0', fontSize:'0.78rem', color:'var(--text-muted)', fontWeight:600 }}>
              {isPrePopulated
                ? `${t.le_logging_closure_for}${ctx.eventLabel}`
                : t.le_every_closed_event}
            </p>
          </div>
        </div>
        {stats?.last_updated && (
          <div className="le-last-updated">
            ...
            {t.le_last_retrain}{TimeSince(stats.last_updated)}
          </div>
        )}
      </div>
          ...
          <div style={{ marginBottom:'24px' }}>
            ...
            {isPrePopulated ? (
              <div style={{ background:'var(--bg-primary)', borderRadius:'12px', padding:'16px 18px' }}>
                <InfoRow label={t.le_event_id_type} value={ctx.eventLabel || '—'} highlight="var(--primary)" />
                <InfoRow label={t.le_officer_id} value={officerId} />
                <InfoRow label={t.le_predicted_clearance} value={`${predicted} ${language === 'kn' ? 'ನಿಮಿಷ' : 'min'}`} highlight="#007aff" />
                <InfoRow label={t.le_barricades_deployed}
                  value={ctx.barricades?.length > 0
                    ? ctx.barricades.map(b => `${b.barricades_needed} ${language === 'kn' ? 'ಸ್ಥಳ' : 'at'} ${b.name}`).join(', ')
                    : t.le_none_predicted}
                />
                <InfoRow label={t.le_officers_assigned} value={ctx.officers ? `${ctx.officers} ${language === 'kn' ? 'ಅಧಿಕಾರಿಗಳು' : 'officers'}` : '—'} />
                {ctx.alternatives?.length > 0 && (
                  <InfoRow label={t.le_diverted_via} value={ctx.alternatives.join(', ')} />
                )}
              </div>
            ) : (
              ...
              <div style={{ background:'var(--bg-primary)', borderRadius:'12px', padding:'14px 16px' }}>
                <p style={{ fontSize:'0.78rem', color:'var(--text-muted)', fontWeight:600, margin:'0 0 12px' }}>
                  {t.le_no_active_event}
                </p>
                <div className="form-group-grid">
                  <div className="form-group">
                    <div className="form-label-with-icon">
                      <label>{t.le_corridor}</label>
                    </div>
                    <select value={standaloneForm.corridor}
                      onChange={e => setStandaloneForm(f => ({ ...f, corridor: e.target.value }))}>
                      {corridors.map(c => <option key={c} value={c}>{language === 'kn' ? (corridorTranslationsKn[c] || c) : c}</option>)}
                    </select>
                  </div>
                  <div className="form-group">
                    <div className="form-label-with-icon">
                      <label>{t.le_event_cause}</label>
                    </div>
                    <select value={standaloneForm.event_cause}
                      onChange={e => setStandaloneForm(f => ({ ...f, event_cause: e.target.value }))}>
                      {causes.map(c => <option key={c} value={c}>{t['cause_' + c] || c.replace(/_/g,' ')}</option>)}
                    </select>
                  </div>
                </div>
                <div className="form-group">
                  <div className="form-label-with-icon"><label>{t.le_ml_predicted_clearance}</label></div>
                ...
                <div className="form-group-grid" style={{ marginTop: '10px' }}>
                  <div className="form-group">
                    <div className="form-label-with-icon">
                      ...
                      <label>{t.le_officers_assigned}</label>
                    </div>
                  ...
                  <div className="form-group">
                    <div className="form-label-with-icon">
                      ...
                      <label>{t.le_barricades_deployed}</label>
                    </div>
          ...
          <div>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'18px', flexWrap:'wrap', gap:'8px' }}>
              ...
              >
                ...
                {isListening ? t.le_listening : voiceLoading ? t.le_parsing_voice : t.le_voice_input}
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group" style={{ marginBottom:'16px' }}>
                <div className="form-label-with-icon">
                  ...
                  <label>{t.le_actual_clearance}</label>
                </div>
              ...
              {hasBoth && (
                <div className="le-delta-card" style={{ marginBottom:'16px' }}>
                  <div className="le-delta-item">
                    <span className="le-delta-label">{t.le_predicted}</span>
                    <span className="le-delta-value" style={{ color:'#007aff' }}>{predicted} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'}</span>
                  </div>
                  ...
                  <div className="le-delta-item">
                    <span className="le-delta-label">{t.le_actual}</span>
                    <span className="le-delta-value" style={{ color:'#34c759' }}>{actual} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'}</span>
                  </div>
                  <div className="le-delta-divider" />
                  <div className="le-delta-item">
                    <span className="le-delta-label">{t.le_delta}</span>
                    <span className="le-delta-value" style={{ color: delta > 0 ? '#ff3b30' : '#34c759' }}>
                      {delta > 0 ? '+' : ''}{delta} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'}
                    </span>
                  </div>
                  <div className="le-delta-item le-delta-accuracy">
                    <span className="le-delta-label">{t.le_accuracy}</span>
              ...
              <div className="form-group" style={{ marginBottom:'16px' }}>
                <div className="form-label-with-icon">
                  ...
                  <label>{t.le_final_location}</label>
                </div>
                <div style={{ display:'flex', gap:'8px' }}>
                  <input type="text" placeholder={language === 'kn' ? 'ಉದಾ. ಮೈಸೂರು ರಸ್ತೆ ಕೆಳಸೇತುವೆ ಹತ್ತಿರ, ಕಿಮೀ 4.2' : 'e.g. Near Mysore Road underpass, km 4.2'}
              ...
              <div style={{ marginBottom:'16px' }}>
                <div className="form-label-with-icon" style={{ marginBottom:'10px' }}>
                  ...
                  <label>{t.le_was_diversion_effective}</label>
                </div>
                <div style={{ display:'flex', gap:'8px' }}>
                  {['Perfect', 'Adequate', 'Failed'].map(opt => (
                    <button key={opt} type="button"
                      ...
                      {opt === 'Perfect' ? '✓ ' : opt === 'Adequate' ? '~ ' : '✗ '}{opt === 'Perfect' ? t.le_perfect : opt === 'Adequate' ? t.le_adequate : t.le_failed}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="form-group" style={{ marginBottom:'16px' }}>
                <div className="form-label-with-icon">
                  ...
                  <label>{t.le_was_manpower_sufficient}</label>
                </div>
                <select value={manpowerSufficiency} onChange={e => setManpowerSufficiency(e.target.value)}>
                  <option value="">{t.le_select_assessment}</option>
                  <option value="Understaffed">{t.le_understaffed}</option>
                  <option value="Just Right">{t.le_just_right}</option>
                  <option value="Overstaffed">{t.le_overstaffed}</option>
                </select>
              </div>
              ...
              {isDelayed && (
                <div className="form-group" style={{ marginBottom:'16px', padding:'14px 16px', background:'rgba(255,59,48,0.05)', borderRadius:'10px', border:'1px solid rgba(255,59,48,0.2)' }}>
                  <div className="form-label-with-icon">
                    ...
                    <label style={{ color:'#ff3b30' }}>{t.le_primary_delay_reason.replace('{delta}', delta)}</label>
                  </div>
                  <select value={delayReason} onChange={e => setDelayReason(e.target.value)}>
                    <option value="">{t.le_select_delay_reason}</option>
                    <option value="Tow Truck Delayed">{t.le_tow_truck_delayed}</option>
                    <option value="Secondary Accident">{t.le_secondary_accident}</option>
                    <option value="Heavy Rain">{t.le_heavy_rain}</option>
                    <option value="Public Interference">{t.le_public_interference}</option>
                    <option value="Equipment Failure">{t.le_equipment_failure}</option>
                    <option value="VIP Movement">{t.le_vip_movement_conflict}</option>
                  </select>
                </div>
              )}
              ...
              <div className="form-group" style={{ marginBottom:'16px' }}>
                <div className="form-label-with-icon">
                  <label>{t.le_additional_notes}</label>
                </div>
                <input type="text" placeholder={language === 'kn' ? 'ಇತರ ಯಾವುದೇ ಅವಲೋಕನಗಳು...' : 'Any other observations...'}
              ...
              {result && (
                <div className="le-success-banner" style={{ marginBottom:'16px' }}>
                  ...
                  <div>
                    <div><strong>{t.le_logged_recalibrated}</strong> {result.message}</div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'20px' }}>
          <div className="card le-ring-card">
            <div className="le-ring-header">
              <div>
                <h3 style={{ margin:0, fontSize:'0.95rem', fontWeight:800, color:'var(--primary)', textTransform:'uppercase' }}>{t.le_model_confidence}</h3>
                <p style={{ margin:'4px 0 0', fontSize:'0.75rem', color:'var(--text-muted)', fontWeight:600 }}>
                  {t.le_based_on_events.replace('{count}', stats?.total_events_logged || 0)}
                </p>
              </div>
              <div className="le-tag" style={{ fontSize:'0.6rem' }}>{t.le_live}</div>
            </div>

            {statsLoading ? (
              <div style={{ textAlign:'center', padding:'32px', color:'var(--text-muted)' }}>{t.le_loading}</div>
              ...
              <div className="le-ring-body">
                ...
                <div className="le-ring-stats">
                  <div className="le-stat-pill">
                    <span className="le-stat-pill-label">{t.le_mae}</span>
                    <span className="le-stat-pill-value" style={{ color:'#7c3aed' }}>±{stats.mae_min} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'}</span>
                  </div>
                  <div className="le-stat-pill">
                    <span className="le-stat-pill-label">{t.le_events_logged}</span>
                    <span className="le-stat-pill-value" style={{ color:'#007aff' }}>{stats.total_events_logged}</span>
                  </div>
                  <div className="le-stat-pill">
                    <span className="le-stat-pill-label">{t.le_last_retrain_val}</span>
            ...
            {stats?.accuracy_by_corridor?.length > 0 && (
              <div style={{ marginTop:'20px', borderTop:'1px solid var(--border-color)', paddingTop:'16px' }}>
                <p style={{ fontSize:'0.7rem', fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'10px' }}>
                  {t.le_accuracy_by_corridor}
                </p>
                {stats.accuracy_by_corridor.map((c, i) => (
                  <div key={i} style={{ marginBottom:'8px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'3px' }}>
                      <span style={{ fontSize:'0.75rem', fontWeight:700, color:'var(--text-dark)' }}>{language === 'kn' ? (corridorTranslationsKn[c.corridor] || c.corridor) : c.corridor}</span>
          ...
          {stats?.recent_feedback?.length > 0 && (
            <div className="card" style={{ padding:'20px 24px' }}>
              <p style={{ fontSize:'0.7rem', fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'14px' }}>
                {t.le_recent_closures}
              </p>
              <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                {stats.recent_feedback.map((fb, i) => (
                  <div key={i} className="le-history-row">
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontWeight:700, fontSize:'0.82rem', color:'#1d1d1f', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{language === 'kn' ? (corridorTranslationsKn[fb.corridor] || fb.corridor) : fb.corridor}</div>
                      <div style={{ fontSize:'0.7rem', color:'var(--text-muted)', fontWeight:600 }}>
                        {t['cause_' + fb.event_cause] || fb.event_cause?.replace(/_/g,' ')} · {fb.predicted_time_min}→{fb.actual_time_min} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'}
                      </div>
                    </div>
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'2px', flexShrink:0 }}>
                      <AccuracyBadge pct={fb.accuracy_pct} />
                      <span style={{ fontSize:'0.65rem', color:'var(--text-muted)', fontWeight:600 }}>
                        {fb.delta_min > 0 ? '+' : ''}{fb.delta_min} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'}
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <div style={{
                ...
                {retrainStatus.post_retrain_mae_min && (
                  <span style={{ marginLeft:'8px', opacity:0.8 }}>
                    · {language === 'kn' ? 'ಹೊಸ MAE:' : 'New MAE:'} ±{retrainStatus.post_retrain_mae_min} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'}
              ...
              <div>
                ...
                {retrainLog.slice(0, 3).map((run, i) => (
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:'1px solid var(--bg-primary)', fontSize:'0.73rem' }}>
                    ...
                    <span style={{ fontWeight:700, color:'#7c3aed' }}>{run.rows_incorporated} {t.le_rows}</span>
                    <span style={{ fontWeight:800, color: run.post_retrain_mae_min < 15 ? '#34c759' : '#ff9500' }}>
                      ±{run.post_retrain_mae_min} {language === 'kn' ? 'ನಿಮಿಷ' : 'min'} MAE
                    </span>
                  </div>
                ))}
              </div>
